...
#Updated: Assumes kubectl is in the path
...
source "${KUBE_ROOT}/scripts/kube-env.sh"
...
while true; do
  ...
  nodes_status=$(kubectl get nodes -o template --template='{{range .items}}{{range .status.conditions}}{{if eq .type "Ready"}}{{.type}}:{{.status}},{{end}}{{end}}{{end}}' --api-version=v1) || true
...
while true; do
  ...
  cs_status=$(kubectl get componentstatuses -o template --template='{{range .items}}{{with index .conditions 0}}{{.type}}:{{.status}},{{end}}{{end}}' --api-version=v1) || true
