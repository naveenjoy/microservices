...
if [ "$cert_ip" == "_use_aws_external_ip_" -o "$cert_ip" == "_use_openstack_external_ip_" ]; then
...
cp -p pki/issued/kubecfg.crt "${cert_dir}/cert.pem"
# cp -p pki/private/kubecfg.key "${cert_dir}/kubecfg.key"
cp -p pki/private/kubecfg.key "${cert_dir}/key.pem"
# Make server certs accessible to apiserver.
chgrp $cert_group "${cert_dir}/server.key" "${cert_dir}/server.cert" "${cert_dir}/ca.pem" "${cert_dir}/cert.pem" "${cert_dir}/key.pem"
chmod 660 "${cert_dir}/server.key" "${cert_dir}/server.cert" "${cert_dir}/ca.pem" "${cert_dir}/cert.pem" "${cert_dir}/key.pem"